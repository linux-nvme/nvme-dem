...
MG_INTERNAL void mg_call (struct mg_connection *nc,
			  mg_event_handler_t ev_handler, int ev,
			  void *ev_data);
...
MG_INTERNAL void mg_http_free_proto_data_cgi (struct mg_http_proto_data_cgi
					      *d);
#endif
...
void
cs_log_print_prefix (const char *func)
  WEAK;
     void cs_log_print_prefix (const char *func)
{
  char prefix[21];
 ...
 */
static void
cs_base64_emit_code (struct cs_base64_ctx *ctx, int v)
{
  if (v < NUM_UPPERCASES)
    {
...
static void
cs_base64_emit_chunk (struct cs_base64_ctx *ctx)
{
  int a, b, c;
...
void
cs_base64_init (struct cs_base64_ctx *ctx, cs_base64_putc_t b64_putc,
		void *user_data)
{
  ctx->chunk_size = 0;
...
void
cs_base64_finish (struct cs_base64_ctx *ctx)
{
  if (ctx->chunk_size > 0)
    {
      int i;
...
void
cs_base64_encode (const unsigned char *src, int src_len, char *dst)
{
  BASE64_ENCODE_BODY;
}
...
void
cs_fprint_base64 (FILE * f, const unsigned char *src, int src_len)
{
  BASE64_ENCODE_BODY;
}
...
void
MD5_Update (MD5_CTX * ctx, const unsigned char *buf, size_t len)
{
  uint32_t t;
...
void
MD5_Final (unsigned char digest[16], MD5_CTX * ctx)
{
  unsigned count;
  unsigned char *p;
  uint32_t *a;
...
void
mbuf_init (struct mbuf *mbuf, size_t initial_size)
  WEAK;
...
void
mbuf_resize (struct mbuf *a, size_t new_size)
  WEAK;
...
size_t
mbuf_insert (struct mbuf *a, size_t off, const void *buf, size_t)
  WEAK;
     size_t mbuf_insert (struct mbuf *a, size_t off, const void *buf,
			 size_t len)
{
  char *p = NULL;
...
size_t
mbuf_append (struct mbuf * a, const void *buf, size_t len)
  WEAK;
...
void
mbuf_remove (struct mbuf *mb, size_t n)
  WEAK;
...
struct mg_str
mg_mk_str_n (const char *s, size_t len)
  WEAK;
...
int
mg_vcmp (const struct mg_str *str1, const char *str2)
  WEAK;
...
int
mg_vcasecmp (const struct mg_str *str1, const char *str2)
  WEAK;
...
int
mg_strncmp (const struct mg_str, const struct mg_str, size_t n)
  WEAK;
     int mg_strncmp (const struct mg_str str1, const struct mg_str str2,
		     size_t n)
{
...
blk0 (union char64long16 *block, int i)
{
  ...
  block->l[i] =
    (rol (block->l[i], 24) & 0xFF00FF00) | (rol (block->l[i], 8) &
					    0x00FF00FF);
#endif
  return block->l[i];
}
...
void
cs_sha1_transform (uint32_t state[5], const unsigned char buffer[64])
{
  uint32_t a, b, c, d, e;
...
void
cs_sha1_init (cs_sha1_ctx * context)
{
...
void
cs_sha1_update (cs_sha1_ctx * context, const unsigned char *data,
		uint32_t len)
{
  uint32_t i, j;
...
void
cs_sha1_final (unsigned char digest[20], cs_sha1_ctx * context)
{
  unsigned i;
...
void
cs_hmac_sha1 (const unsigned char *key, size_t keylen,
...
size_t
c_strnlen (const char *s, size_t maxlen)
  WEAK;
...
int
c_vsnprintf (char *buf, size_t buf_size, const char *fmt, va_list ap)
  WEAK;
...
#else
static int
c_itoa (char *buf, size_t buf_size, int64_t num, int base, int flags,
	int field_width)
{
  char tmp[40];
  int i = 0, k = 0, neg = 0;
	  ...
	  if (ch == 's')
	    {
	      ...
	      int j;
	      int pad =
		field_width - (precision >= 0 ? c_strnlen (s, precision) : 0);
	      for (j = 0; j < pad; j++)
		{
	    ...
	    {
	      i +=
		c_itoa (buf + i, buf_size - i, va_arg (ap, long), 10, flags,
			field_width);
#ifdef SSIZE_MAX
	    }
	    ...
	    {
	      i +=
		c_itoa (buf + i, buf_size - i, va_arg (ap, ssize_t), 10,
			flags, field_width);
#endif
	    }
	    ...
	    {
	      i +=
		c_itoa (buf + i, buf_size - i, va_arg (ap, int64_t), 10,
			flags, field_width);
	    }
...
int
c_snprintf (char *buf, size_t buf_size, const char *fmt, ...)
  WEAK;
...
const char *
c_strnstr (const char *s, const char *find, size_t slen)
  WEAK;
...
void
cs_to_hex (char *to, const unsigned char *p, size_t len)
  WEAK;
...
void
cs_from_hex (char *to, const char *p, size_t len)
  WEAK;
...
static int
str_util_lowercase (const char *s)
{
...
int
mg_asprintf (char **buf, size_t size, const char *fmt, ...)
  WEAK;
...
int
mg_avprintf (char **buf, size_t size, const char *fmt, va_list ap)
  WEAK;
...
MG_INTERNAL void
mg_add_conn (struct mg_mgr *mgr, struct mg_connection *c)
{
  DBG (("%p %p", mgr, c));
  c->mgr = mgr;
...
MG_INTERNAL void
mg_remove_conn (struct mg_connection *conn)
{
  if (conn->prev == NULL)
...
MG_INTERNAL void
mg_call (struct mg_connection *nc,
  ...
  if (ev != MG_EV_POLL)
    {
      DBG (("%p %s ev=%d ev_data=%p flags=%lu rmbl=%d smbl=%d", nc,
	    ev_handler == nc->handler ? "user" : "proto", ev, ev_data,
...
void
mg_if_timer (struct mg_connection *c, double now)
{
...
void
mg_if_poll (struct mg_connection *nc, time_t now)
{
...
static void
mg_destroy_conn (struct mg_connection *conn, int destroy_if)
{
  if (destroy_if)
...
void
mg_close_conn (struct mg_connection *conn)
{
...
void
mg_mgr_init (struct mg_mgr *m, void *user_data)
{
...
void
mg_mgr_init_opt (struct mg_mgr *m, void *user_data,
		 struct mg_mgr_init_opts opts)
{
  memset (m, 0, sizeof(*m));
...
static enum v7_err
mg_send_js (struct v7 *v7, v7_val_t * res)
{
...
enum v7_err
mg_enable_javascript (struct mg_mgr *m, struct v7 *v7,
...
time_t
mg_mgr_poll (struct mg_mgr *m, int timeout_ms)
{
  int i;
...
int
mg_vprintf (struct mg_connection *nc, const char *fmt, va_list ap)
{
...
static int
mg_resolve2 (const char *host, struct in_addr *ina)
{
#if MG_ENABLE_GETADDRINFO
  int rv = 0;
  struct addrinfo hints, *servinfo, *p;
  struct sockaddr_in *h = NULL;
  memset (&hints, 0, sizeofhints);
  ...
  if ((he = gethostbyname (host)) == NULL)
    {
      DBG (("gethostbyname(%s) failed: %s", host,
...
int
mg_resolve (const char *host, char *buf, size_t n)
{
  struct in_addr ad;
  return mg_resolve2 (host, &ad) ? snprintf (buf, n, "%s",
					     inet_ntoa (ad)) : 0;
}
 ...
 */
MG_INTERNAL int
mg_parse_address (const char *str, union socket_address *sa,
		  int *proto, char *host, size_t host_len)
{
  ...
  *proto = SOCK_STREAM;
  ...
  return port < 0xffffUL && (ch == '\0' || ch == ','
			     || isspace (ch)) ? len : -1;
}

struct mg_connection *
mg_if_accept_new_conn (struct mg_connection *lc)
{
  struct mg_add_sock_opts opts;
...
void
mg_if_accept_tcp_cb (struct mg_connection *nc, union socket_address *sa,
		     size_t sa_len)
{
  (void) sa_len;
  nc->sa = *sa;
...
void
mg_send (struct mg_connection *nc, const void *buf, int len)
{
...
void
mg_if_sent_cb (struct mg_connection *nc, int num_sent)
{
  if (num_sent < 0)
    {
...
MG_INTERNAL void
mg_recv_common (struct mg_connection *nc, void *buf, int len, int own)
{
...
void
mg_if_recv_tcp_cb (struct mg_connection *nc, void *buf, int len, int own)
{
  mg_recv_common (nc, buf, len, own);
}

void
mg_if_recv_udp_cb (struct mg_connection *nc, void *buf, int len,
		   union socket_address *sa, size_t sa_len)
{
  ...
  if (nc->flags & MG_F_LISTENING)
	...
	*/
      for (nc = mg_next (lc->mgr, NULL); nc != NULL;
	   nc = mg_next (lc->mgr, nc))
	{
...
void
mg_if_connect_cb (struct mg_connection *nc, int err)
{
 ...
 */
static void
resolve_cb (struct mg_dns_message *msg, void *data, enum mg_resolve_err e)
{
...
struct mg_connection *
mg_connect (struct mg_mgr *mgr, const char *address,
	    mg_event_handler_t callback)
{
...
struct mg_connection *
mg_connect_opt (struct mg_mgr *mgr, const char *address,
		mg_event_handler_t callback, struct mg_connect_opts opts)
{
  ...
  if ((rc =
	mg_parse_address (address, &nc->sa, &proto, host, sizeof(host))) < 0)
    {
  ...
  if (rc == 0)
	...
	*/
      ...
      memset (&o, 0, sizeof(o));
      o.dns_conn = &dns_conn;
      if (mg_resolve_async_opt
	  (nc->mgr, host, MG_DNS_A_RECORD, resolve_cb, nc, o) != 0)
	{
...
struct mg_connection *
mg_bind (struct mg_mgr *srv, const char *address,
...
struct mg_connection *
mg_bind_opt (struct mg_mgr *mgr, const char *address,
	     mg_event_handler_t callback, struct mg_bind_opts opts)
{
  union socket_address sa;
  struct mg_connection *nc = NULL;
  int proto, rc;
...
struct mg_connection *
mg_next (struct mg_mgr *s, struct mg_connection *conn)
{
...
#if MG_ENABLE_BROADCAST
void
mg_broadcast (struct mg_mgr *mgr, mg_event_handler_t cb, void *data,
	      size_t len)
{
...
int
mg_check_ip_acl (const char *acl, uint32_t remote_ip)
{
  int allowed, flag;
  uint32_t net, mask;
  struct mg_str vec;
...
void
mg_forward (struct mg_connection *from, struct mg_connection *to)
{
...
double
mg_set_timer (struct mg_connection *c, double timestamp)
{
...
void
mg_sock_set (struct mg_connection *nc, sock_t sock)
{
...
void
mg_if_get_conn_addr (struct mg_connection *nc, int remote,
		     union socket_address *sa)
{
...
struct mg_connection *
mg_add_sock_opt (struct mg_mgr *s, sock_t sock,
		 mg_event_handler_t callback, struct mg_add_sock_opts opts)
{
...
struct mg_connection *
mg_add_sock (struct mg_mgr *s, sock_t sock, mg_event_handler_t callback)
{
  struct mg_add_sock_opts opts;
  memset (&opts, 0, sizeof(opts));
...
struct mg_iface *
mg_if_create_iface (struct mg_iface_vtable *vtable, struct mg_mgr *mgr)
{
...
struct mg_iface *
mg_find_iface (struct mg_mgr *mgr,
		struct mg_iface_vtable *vtable, struct mg_iface *from)
{
  int i = 0;
  if (from != NULL)
    {
...
void
mg_set_non_blocking_mode (sock_t sock)
{
#ifdef _WIN32
  unsigned long on = 1;
...
void
mg_socket_if_connect_tcp (struct mg_connection *nc,
...
void
mg_socket_if_connect_udp (struct mg_connection *nc)
{
...
int
mg_socket_if_listen_tcp (struct mg_connection *nc, union socket_address *sa)
{
  int proto = 0;
...
int
mg_socket_if_listen_udp (struct mg_connection *nc, union socket_address *sa)
{
...
void
mg_socket_if_tcp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
}

void
mg_socket_if_udp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
}

void
mg_socket_if_recved (struct mg_connection *nc, size_t len)
{
  (void) nc;
  (void) len;
}
...
int
mg_socket_if_create_conn (struct mg_connection *nc)
{
  (void) nc;
  return 1;
}
...
void
mg_socket_if_destroy_conn (struct mg_connection *nc)
{
  if (nc->sock == INVALID_SOCKET)
    return;
  if (!(nc->flags & MG_F_UDP))
...
static int
mg_accept_conn (struct mg_connection *lc)
{
  struct mg_connection *nc;
  union socket_address sa;
...
static sock_t
mg_open_listening_socket (union socket_address *sa, int type, int proto)
{
  socklen_t sa_len =
...
static void
mg_write_to_socket (struct mg_connection *nc)
{
  ...
  if (nc->flags & MG_F_UDP)
    {
      int n =
	sendto (nc->sock, io->buf, io->len, 0, &nc->sa.sa,
		sizeof(nc->sa.sin));
...
MG_INTERNAL size_t
recv_avail_size (struct mg_connection *conn, size_t max)
{
  size_t avail;
...
static void
mg_handle_tcp_read (struct mg_connection *conn)
{
  int n = 0;
  ...
  if (conn->flags & MG_F_SSL)
      ...
      if (conn->flags & MG_F_SSL_HANDSHAKE_DONE)
	{
	  ...
	  while ((n =
		  mg_ssl_if_read (conn, buf, MG_TCP_RECV_BUFFER_SIZE)) > 0)
	    {
...
static int
mg_recvfrom (struct mg_connection *nc, union socket_address *sa,
...
static void
mg_handle_udp_read (struct mg_connection *nc)
{
  char *buf = NULL;
  union socket_address sa;
  socklen_t sa_len = sizeof(sa);
...
#if MG_ENABLE_SSL
static void
mg_ssl_begin (struct mg_connection *nc)
{
...
void
mg_mgr_handle_conn (struct mg_connection *nc, int fd_flags, double now)
{
  int worth_logging =
	  ...
	  if (!(nc->flags & MG_F_UDP))
	    {
	      ...
	      int ret =
		getsockopt (nc->sock, SOL_SOCKET, SO_ERROR, (char *) &err,
			    &len);
	      if (ret != 0)
		{
		  err = 1;
  ...
  if (worth_logging)
    {
      DBG (("%p after fd=%d nc_flags=%lu rmbl=%d smbl=%d", nc, nc->sock,
	    nc->flags, (int) nc->recv_mbuf.len, (int) nc->send_mbuf.len));
    }
}
...
#if MG_ENABLE_BROADCAST
static void
mg_mgr_handle_ctl_sock (struct mg_mgr *mgr)
{
  struct ctl_msg ctl_msg;
  int len =
...
void
mg_socket_if_sock_set (struct mg_connection *nc, sock_t sock)
{
  mg_set_non_blocking_mode (sock);
...
void
mg_socket_if_init (struct mg_iface *iface)
{
  (void) iface;
...
void
mg_socket_if_free (struct mg_iface *iface)
{
  (void) iface;
}

void
mg_socket_if_add_conn (struct mg_connection *nc)
{
  (void) nc;
}

void
mg_socket_if_remove_conn (struct mg_connection *nc)
{
  (void) nc;
}

void
mg_add_to_set (sock_t sock, fd_set * set, sock_t * max_fd)
{
  if (sock != INVALID_SOCKET
#ifdef __unix__
...
mg_socket_if_poll (struct mg_iface *iface, int timeout_ms)
{
  ...
  now = mg_time ();
#if 0
  DBG (("select @ %ld num_ev=%d of %d, timeout=%d", (long) now, num_ev,
	num_fds, timeout_ms));
#endif
...
#if MG_ENABLE_BROADCAST
int
mg_socketpair (sock_t sp[2], int sock_type)
{
  union socket_address sa;
  sock_t sock;
...
static void
mg_sock_get_addr (sock_t sock, int remote, union socket_address *sa)
{
...
void
mg_sock_to_str (sock_t sock, char *buf, size_t len, int flags)
{
  union socket_address sa;
...
void
mg_socket_if_get_conn_addr (struct mg_connection *nc, int remote,
			    union socket_address *sa)
{
...
void
mg_tun_if_connect_tcp (struct mg_connection *nc,
			const union socket_address *sa)
{
  (void) nc;
  (void) sa;
...
void
mg_tun_if_connect_udp (struct mg_connection *nc)
{
  (void) nc;
}

int
mg_tun_if_listen_tcp (struct mg_connection *nc, union socket_address *sa)
{
  (void) nc;
  (void) sa;
  return 0;
...
int
mg_tun_if_listen_udp (struct mg_connection *nc, union socket_address *sa)
{
  (void) nc;
  (void) sa;
  return -1;
...
void
mg_tun_if_tcp_send (struct mg_connection *nc, const void *buf, size_t len)
{
...
void
mg_tun_if_udp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  (void) nc;
  (void) buf;
  (void) len;
...
void
mg_tun_if_recved (struct mg_connection *nc, size_t len)
{
  (void) nc;
  (void) len;
}
...
int
mg_tun_if_create_conn (struct mg_connection *nc)
{
  (void) nc;
  return 1;
}
...
void
mg_tun_if_destroy_conn (struct mg_connection *nc)
{
  struct mg_tun_client *client = (struct mg_tun_client *) nc->iface->data;
...
/* Associate a socket to a connection. */
void
mg_tun_if_sock_set (struct mg_connection *nc, sock_t sock)
{
  (void) nc;
  (void) sock;
}
...
void
mg_tun_if_init (struct mg_iface *iface)
{
  (void) iface;
}

void
mg_tun_if_free (struct mg_iface *iface)
{
  (void) iface;
}

void
mg_tun_if_add_conn (struct mg_connection *nc)
{
  nc->sock = INVALID_SOCKET;
}

void
mg_tun_if_remove_conn (struct mg_connection *nc)
{
  (void) nc;
}

time_t
mg_tun_if_poll (struct mg_iface *iface, int timeout_ms)
{
  (void) iface;
  (void) timeout_ms;
...
void
mg_tun_if_get_conn_addr (struct mg_connection *nc, int remote,
			 union socket_address *sa)
{
  (void) nc;
  (void) remote;
...
struct mg_connection *
mg_tun_if_find_conn (struct mg_tun_client *client, uint32_t stream_id)
{
  struct mg_connection *nc = NULL;
  ...
  if (stream_id > client->last_stream_id)
    ...
    {
      LOG (LL_DEBUG,
	   ("Ignoring stream 0x%lx (last_stream_id 0x%lx)", stream_id,
...
mg_ssl_if_conn_accept (struct mg_connection *nc, struct mg_connection *lc)
{
...
enum mg_ssl_if_result
mg_ssl_if_conn_init (struct mg_connection *nc,
...
static enum mg_ssl_if_result
mg_ssl_if_ssl_err (struct mg_connection *nc, int res)
{
...
enum mg_ssl_if_result
mg_ssl_if_handshake (struct mg_connection *nc)
{
  struct mg_ssl_if_ctx *ctx = (struct mg_ssl_if_ctx *) nc->ssl_if_data;
  int server_side = (nc->listener != NULL);
  int res;
...
int
mg_ssl_if_read (struct mg_connection *nc, void *buf, size_t buf_size)
{
  struct mg_ssl_if_ctx *ctx = (struct mg_ssl_if_ctx *) nc->ssl_if_data;
...
int
mg_ssl_if_write (struct mg_connection *nc, const void *data, size_t len)
{
  struct mg_ssl_if_ctx *ctx = (struct mg_ssl_if_ctx *) nc->ssl_if_data;
...
void
mg_ssl_if_conn_free (struct mg_connection *nc)
{
  struct mg_ssl_if_ctx *ctx = (struct mg_ssl_if_ctx *) nc->ssl_if_data;
  if (ctx == NULL)
    return;
...
static enum mg_ssl_if_result
mg_use_ca_cert (SSL_CTX * ctx, const char *cert)
{
  ...
  SSL_CTX_set_verify (ctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
		      0);
  return SSL_CTX_load_verify_locations (ctx, cert,
					NULL) == 1 ? MG_SSL_OK : MG_SSL_ERROR;
}

static enum mg_ssl_if_result
mg_use_cert (SSL_CTX * ctx, const char *cert,
...
static enum mg_ssl_if_result
mg_set_cipher_list (SSL_CTX * ctx, const char *cl)
{
...
const char *
mg_set_ssl (struct mg_connection *nc, const char *cert, const char *ca_cert)
{
  const char *err_msg = NULL;
  struct mg_ssl_if_conn_params params;
  memset (&params, 0, sizeof(params));
...
static void
mg_ssl_mbed_log (void *ctx, int level, const char *file, int line,
		 const char *str)
{
		     ...
		     const char **err_msg)
{
  ...
  if (mbedtls_ssl_config_defaults
      (ctx->conf,
	(nc->
	flags & MG_F_LISTENING ? MBEDTLS_SSL_IS_SERVER :
...
#else
static int
ssl_socket_send (void *ctx, const unsigned char *buf, size_t len)
{
  ...
  n = mg_get_errno ();
  return ((n == EAGAIN
	   || n == EINPROGRESS) ? MBEDTLS_ERR_SSL_WANT_WRITE : -1);
}

static int
ssl_socket_recv (void *ctx, unsigned char *buf, size_t len)
{
  struct mg_connection *nc = (struct mg_connection *) ctx;
...
static enum mg_ssl_if_result
mg_ssl_if_mbed_err (struct mg_connection *nc, int ret)
{
...
static void
mg_ssl_if_mbed_free_certs_and_keys (struct mg_ssl_if_ctx *ctx)
{
  if (ctx->cert != NULL)
    {
...
mg_ssl_if_handshake (struct mg_connection *nc)
{
  ...
  if (ctx->ssl->p_bio == NULL)
    {
      mbedtls_ssl_set_bio (ctx->ssl, nc, ssl_socket_send, ssl_socket_recv,
			   NULL);
    }
...
static enum mg_ssl_if_result
mg_use_ca_cert (struct mg_ssl_if_ctx *ctx, const char *ca_cert)
{
...
static enum mg_ssl_if_result
mg_use_cert (struct mg_ssl_if_ctx *ctx,
	     const char *cert, const char *key, const char **err_msg)
{
  if (key == NULL)
    key = cert;
  if (cert == NULL || cert[0] == '\0' || key == NULL || key[0] == '\0')
 ...
 */
static enum mg_ssl_if_result
mg_set_cipher_list (struct mg_ssl_if_ctx *ctx, const char *ciphers)
{
  if (ciphers != NULL)
    {
      int l, id;
...
int
mg_ssl_if_mbed_random (void *ctx, unsigned char *buf, size_t len)
{
  (void) ctx;
  while (len--)
    *buf++ = rand ();
 ...
 */
static void
parse_uri_component (const char **p, const char *end, char sep,
		     struct mg_str *res)
{
  res->p = *p;
  for (; *p < end; (*p)++)
...
int
mg_parse_uri (struct mg_str uri, struct mg_str *scheme,
	      struct mg_str *user_info, struct mg_str *host,
	      unsigned int *port, struct mg_str *path, struct mg_str *query,
	      struct mg_str *fragment)
{
  struct mg_str rscheme = { 0, 0 }, ruser_info =
  {
  0, 0}, rhost =
  {
  0, 0}, rpath =
...
int
mg_normalize_uri_path (const struct mg_str *in, struct mg_str *out)
{
...
#if MG_ENABLE_FILESYSTEM
static void
mg_http_free_proto_data_file (struct mg_http_proto_data_file *d)
{
  if (d != NULL)
    {
      if (d->fp != NULL)
...
static void
mg_http_free_proto_data_endpoints (struct mg_http_endpoint **ep)
{
...
static void
mg_http_free_reverse_proxy_data (struct mg_reverse_proxy_data *rpd)
{
  if (rpd->linked_conn != NULL)
	...
	*/
	...
	mg_http_get_proto_data (rpd->linked_conn);
...
static void
mg_http_conn_destructor (void *proto_data)
{
...
} mg_static_builtin_mime_types[] =
{
  MIME_ENTRY ("html", "text/html"),
    ...
    MIME_ENTRY ("avi", "video/x-msvideo"), MIME_ENTRY ("bmp", "image/bmp"),
  {
NULL, 0, NULL}};

static struct mg_str
mg_get_mime_type (const char *path, const char *dflt,
		  const struct mg_serve_http_opts *opts)
{
 ...
 */
static int
mg_http_get_request_len (const char *s, int buf_len)
{
...
int
mg_parse_http (const char *s, int n, struct http_message *hm, int is_req)
{
  const char *end, *qs;
...
struct mg_str *
mg_get_http_header (struct http_message *hm, const char *name)
{
...
#if MG_ENABLE_FILESYSTEM
static void
mg_http_transfer_file_data (struct mg_connection *nc)
{
 ...
 */
static size_t
mg_http_parse_chunk (char *buf, size_t len, char **chunk_data,
		     size_t * chunk_len)
{
...
MG_INTERNAL size_t
mg_handle_chunked (struct mg_connection * nc,
		   struct http_message * hm, char *buf, size_t blen)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (nc);
  char *data;
  ...
  if (i > body_len)
      ...
      if (zero_chunk_received)
	{
	  ...
	  hm->message.len =
	    (size_t) pd->chunk.body_len + blen - i + (hm->body.p -
						      hm->message.p);
	}
    }
...
static mg_event_handler_t
mg_http_get_endpoint_handler (struct mg_connection *nc,
...
static void
mg_http_call_endpoint_handler (struct mg_connection *nc, int ev,
				struct http_message *hm)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (nc);
  ...
  if (pd->endpoint_handler == NULL || ev == MG_EV_HTTP_REQUEST)
	...
	? mg_http_get_endpoint_handler (nc->listener, &hm->uri) : NULL;
    }
...
void
mg_http_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  struct http_message hm;
...
static void
mg_http_handler2 (struct mg_connection *nc, int ev, void *ev_data,
		  struct http_message *hm)
{
#else /* !__XTENSA__ */
void
mg_http_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  ...
  if (ev == MG_EV_CLOSE)
      ...
      if (pd->mp_stream.boundary != NULL)
	{
	  ...
	  mg_call (nc,
		   (pd->endpoint_handler ? pd->endpoint_handler : nc->
		    handler), MG_EV_HTTP_PART_END, &mp);
	  mp.var_name = NULL;
	  mp.file_name = NULL;
	  mg_call (nc,
		   (pd->endpoint_handler ? pd->endpoint_handler : nc->
		    handler), MG_EV_HTTP_MULTIPART_REQUEST_END, &mp);
	}
      else
#endif
...
static size_t
mg_get_line_len (const char *buf, size_t buf_len)
{
  size_t len = 0;
...
#if MG_ENABLE_HTTP_STREAMING_MULTIPART
static void
mg_http_multipart_begin (struct mg_connection *nc,
			 struct http_message *hm, int req_len)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (nc);
  struct mg_str *ct;
...
static void
mg_http_multipart_call_handler (struct mg_connection *c, int ev,
...
static int
mg_http_multipart_got_chunk (struct mg_connection *c)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (c);
...
static int
mg_http_multipart_finalize (struct mg_connection *c)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (c);
  ...
  mg_http_multipart_call_handler (c, MG_EV_HTTP_MULTIPART_REQUEST_END, NULL,
				  0);
  mg_http_free_proto_data_mp_stream (&pd->mp_stream);
...
static int
mg_http_multipart_wait_for_boundary (struct mg_connection *c)
{
  const char *boundary;
  struct mbuf *io = &c->recv_mbuf;
  struct mg_http_proto_data *pd = mg_http_get_proto_data (c);
...
static int
mg_http_multipart_process_boundary (struct mg_connection *c)
{
  int data_size;
  ...
  while (data_size > 0 &&
      ...
      if (line_len == 2 && mg_ncasecmp (block_begin, "\r\n", 2) == 0)
	{
	  ...
	  if (pd->mp_stream.processing_part != 0)
	    {
	      mg_http_multipart_call_handler (c, MG_EV_HTTP_PART_END, NULL,
					      0);
	    }
...
static void
mg_http_multipart_continue (struct mg_connection *c)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (c);
  while (1)
    {
...
void
mg_set_protocol_http_websocket (struct mg_connection *nc)
{
  nc->proto_handler = mg_http_handler;
}

const char *
mg_status_message (int status_code)
{
  switch (status_code)
    {
    case 206:
...
void
mg_send_response_line_s (struct mg_connection *nc, int status_code,
...
void
mg_send_response_line (struct mg_connection *nc, int status_code,
...
void
mg_http_send_redirect (struct mg_connection *nc, int status_code,
...
void
mg_http_send_error (struct mg_connection *nc, int code, const char *reason)
{
  if (!reason)
...
#if MG_ENABLE_FILESYSTEM
static void
mg_http_construct_etag (char *buf, size_t buf_len, const cs_stat_t * st)
{
...
#ifndef WINCE
static void
mg_gmt_time_string (char *buf, size_t buf_len, time_t * t)
{
...
static int
mg_http_parse_range_header (const struct mg_str *header, int64_t * a,
			    int64_t * b)
{
  /*
...
void
mg_http_serve_file (struct mg_connection *nc, struct http_message *hm,
		    ...
		    const struct mg_str extra_headers)
{
  ...
  cs_stat_t st;
  LOG (LL_DEBUG,
	("%p [%s] %.*s", nc, path, (int) mime_type.len, mime_type.p));
  if (mg_stat (path, &st) != 0
      || (pd->file.fp = mg_fopen (path, "rb")) == NULL)
    {
	...
	if (conn_hdr != NULL)
      ...
      mg_gmt_time_string (last_modified, sizeof(last_modified),
			  &st.st_mtime);
      /*
	...
	*/
      ...
      mg_printf (nc,
		 ...
		 "\r\n"
		 "%sEtag: %s\r\n\r\n",
		 current_time, last_modified, (int) mime_type.len,
		 mime_type.p, (pd->file.keepalive ? "keep-alive" : "close"),
...
static void
mg_http_serve_file2 (struct mg_connection *nc, const char *path,
		     struct http_message *hm, struct mg_serve_http_opts *opts)
{
#if MG_ENABLE_HTTP_SSI
...
#endif
  mg_http_serve_file (nc, hm, path,
		      mg_get_mime_type (path, "text/plain", opts),
...
int
mg_url_decode (const char *src, int src_len, char *dst, int dst_len,
...
int
mg_get_http_var (const struct mg_str *buf, const char *name, char *dst,
		 size_t dst_len)
{
  const char *p, *e, *s;
  size_t name_len;
...
void
mg_printf_html_escape (struct mg_connection *nc, const char *fmt, ...)
{
  char mem[MG_VPRINTF_BUFFER_SIZE], *buf = mem;
  int i, j, len;
  va_list ap;
...
int
mg_http_parse_header (struct mg_str *hdr, const char *var_name, char *buf,
		      size_t buf_size)
{
...
int
mg_get_http_basic_auth (struct http_message *hm, char *user, size_t user_len,
...
int
mg_parse_http_basic_auth (struct mg_str *hdr, char *user, size_t user_len,
			  char *pass, size_t pass_len)
{
  char *buf = NULL;
  char fmt[64];
...
#if MG_ENABLE_FILESYSTEM
static int
mg_is_file_hidden (const char *path,
		   const struct mg_serve_http_opts *opts,
...
static void
mg_mkmd5resp (const char *method, size_t method_len, const char *uri,
...
int
mg_http_create_digest_auth_header (char *buf, size_t buf_len,
				   ...
				   const char *passwd)
{
  ...
  mg_mkmd5resp (method, strlen (method), uri, strlen (uri), ha1,
		sizeof(ha1) - 1, cnonce, strlen (cnonce), "1", one, cnonce,
		strlen (cnonce), qop, sizeof(qop) - 1, resp);
  return snprintf (buf, buf_len,
		   "Authorization: Digest username=\"%s\","
		   "realm=\"%s\",uri=\"%s\",qop=%s,nc=1,cnonce=%s,"
		   "nonce=%s,response=%s\r\n", user, auth_domain, uri, qop,
		   cnonce, cnonce, resp);
}
	 ...
	 */
static int
mg_check_nonce (const char *nonce)
{
...
int
mg_http_check_digest_auth (struct http_message *hm, const char *auth_domain,
			   FILE * fp)
{
  struct mg_str *hdr;
  ...
  while (fgets (buf, sizeof(buf), fp) != NULL)
	  ...
	  strcmp (auth_domain, f_domain) == 0)
	{
	  ...
	  mg_mkmd5resp (hm->method.p, hm->method.len, hm->uri.p,
			hm->uri.len +
			(hm->query_string.len ? hm->query_string.len + 1 : 0),
			f_ha1, strlen (f_ha1), nonce, strlen (nonce), nc,
			strlen (nc), cnonce, strlen (cnonce), qop,
			strlen (qop), expected_response);
...
static int
mg_is_authorized (struct http_message *hm, const char *path,
...
static size_t
mg_url_encode (const char *src, size_t s_len, char *dst, size_t dst_len)
{
...
static void
mg_escape (const char *src, char *dst, size_t dst_len)
{
  size_t n = 0;
...
static void
mg_print_dir_entry (struct mg_connection *nc, const char *file_name,
		    cs_stat_t * stp)
{
...
static void
mg_scan_directory (struct mg_connection *nc, const char *dir,
		   const struct mg_serve_http_opts *opts,
...
static void
mg_send_directory_listing (struct mg_connection *nc, const char *dir,
			   struct http_message *hm,
			   struct mg_serve_http_opts *opts)
{
  ...
  mg_printf_http_chunk (nc,
			...
			"</thead>\n"
			"<tbody id=tb>",
			(int) hm->uri.len, hm->uri.p, sort_js_code,
			sort_js_code2, (int) hm->uri.len, hm->uri.p);
  ...
  nc->flags |= MG_F_SEND_AND_CLOSE;
	 ...
	 */
MG_INTERNAL void
mg_find_index_file (const char *path, const char *list,
...
static void
mg_reverse_proxy_handler (struct mg_connection *nc, int ev, void *ev_data)
{
...
void
mg_http_reverse_proxy (struct mg_connection *nc,
			...
			struct mg_str upstream)
{
  ...
  mg_asprintf (&purl, sizeof(burl), "%.*s%.*s", (int) upstream.len,
		upstream.p, (int) (hm->uri.len - mount.len),
		hm->uri.p + mount.len);

  be =
    mg_connect_http_base (nc->mgr, mg_reverse_proxy_handler, opts, "http://",
...
static int
mg_http_handle_forwarding (struct mg_connection *nc,
			   struct http_message *hm,
			   const struct mg_serve_http_opts *opts)
{
  const char *rewrites = opts->url_rewrites;
  ...
  while ((rewrites = mg_next_comma_list_entry (rewrites, &a, &b)) != NULL)
	...
	{
	  if (mg_strncmp (b, p1, p1.len) == 0
	      || mg_strncmp (b, p2, p2.len) == 0)
	    {
...
MG_INTERNAL int
mg_uri_to_local_path (struct http_message *hm,
		      const struct mg_serve_http_opts *opts,
	...
	if (component.len > 0)
	  {
	    ...
	    len =
	      mg_url_decode (p + 1, component.len, p + 1, lp_end - p - 1, 0);
	    if (len <= 0)
	      {
		ok = 0;
		break;
...
static int
mg_get_month_index (const char *s)
{
...
MG_INTERNAL time_t
mg_parse_date_string (const char *datetime)
{
...
MG_INTERNAL int
mg_is_not_modified (struct http_message *hm, cs_stat_t * st)
{
  struct mg_str *hdr;
...
static void
mg_http_send_digest_auth_request (struct mg_connection *c, const char *domain)
{
  mg_printf (c,
...
static void
mg_http_send_options (struct mg_connection *nc)
{
  mg_printf (nc, "%s",
...
static int
mg_is_creation_request (const struct http_message *hm)
{
...
MG_INTERNAL void
mg_send_http_file (struct mg_connection *nc, char *path,
		   const struct mg_str *path_info,
		   struct http_message *hm, struct mg_serve_http_opts *opts)
{
  ...
  LOG (LL_DEBUG,
	...
	(int) hm->method.len, hm->method.p, path, exists, is_directory,
	is_dav, is_cgi, index_file ? index_file : ""));
...
void
mg_serve_http (struct mg_connection *nc, struct http_message *hm,
		struct mg_serve_http_opts opts)
{
  char *path = NULL;
...
#if MG_ENABLE_HTTP_STREAMING_MULTIPART
void
mg_file_upload_handler (struct mg_connection *nc, int ev, void *ev_data,
			mg_fu_fname_fn local_name_fn)
{
  switch (ev)
	...
	if (lfn.p == NULL || lfn.len == 0)
	  {
	    LOG (LL_ERROR,
		 ("%p Not allowed to upload %s", nc, mp->file_name));
	    mg_printf (nc,
	...
	if (mp->status >= 0 && fus->fp != NULL)
	  {
	    LOG (LL_DEBUG,
		 ("%p Uploaded %s (%s), %d bytes", nc, mp->file_name,
		  fus->lfn, (int) fus->num_recd));
	    mg_printf (nc,
			"HTTP/1.1 200 OK\r\n" "Content-Type: text/plain\r\n"
			"Connection: close\r\n\r\n" "Ok, %s - %d bytes.\r\n",
	  ...
	  {
	    LOG (LL_ERROR,
		 ("Failed to store %s (%s)", mp->file_name, fus->lfn));
	    /*
...
MG_INTERNAL int
mg_http_common_url_parse (const char *url, const char *schema,
  ...
  if (user != NULL && pass != NULL)
      ...
      else
	{
	  ...
	  *pass = (char *) MG_MALLOC (auth_sep_pos - user_sep_pos - 1 + 1);
	  memcpy (*pass, *addr + user_sep_pos + 1,
		  auth_sep_pos - user_sep_pos - 1);
...
mg_connect_http_base (struct mg_mgr *mgr, mg_event_handler_t ev_handler,
		      ...
		      const char *schema_ssl, const char *url,
		      const char **path, char **user, char **pass,
		      char **addr)
{
  struct mg_connection *nc = NULL;
  int port_i = -1;
...
struct mg_connection *
mg_connect_http_opt (struct mg_mgr *mgr,
		     mg_event_handler_t ev_handler,
		     struct mg_connect_opts opts,
		     const char *url,
...
size_t
mg_parse_multipart (const char *buf, size_t buf_len, char *var_name,
...
void
mg_register_http_endpoint (struct mg_connection *nc, const char *uri_path,
			   mg_event_handler_t handler)
{
...
static int
mg_wait_until_ready (sock_t sock, int for_read)
{
  fd_set set;
  FD_ZERO (&set);
  FD_SET (sock, &set);
  return select (sock + 1, for_read ? &set : 0, for_read ? 0 : &set, 0,
		 0) == 1;
}

static void *
mg_push_to_stdin (void *arg)
{
...
static void
mg_spawn_stdio_thread (sock_t sock, HANDLE hPipe, void *(*func) (void *))
{
  struct mg_threadparam *tp =
    (struct mg_threadparam *) MG_MALLOC (sizeof(*tp));
  if (tp != NULL)
    {
      tp->s = sock;
      tp->hPipe = hPipe;
...
static void
mg_abs_path (const char *utf8_path, char *abs_path, size_t len)
{
...
static int
mg_start_process (const char *interp, const char *cmd,
		  ...
		  const char *dir, sock_t sock)
{
  ...
  return (pid != 0);
	 ...
	 */
static char *
mg_addenv (struct mg_cgi_env_block *block, const char *fmt, ...)
{
  int n, space;
...
static void
mg_addenv2 (struct mg_cgi_env_block *blk, const char *name)
{
  const char *s;
...
static void
mg_prepare_cgi_environment (struct mg_connection *nc,
			    const char *prog,
			    const struct mg_str *path_info,
			    const struct http_message *hm,
			    const struct mg_serve_http_opts *opts,
			    struct mg_cgi_env_block *blk)
{
  ...
  if (*s == '/')
    ...
    {
      mg_addenv (blk, "SCRIPT_NAME=%.*s", (int) (s - hm->uri.p + 1),
		 hm->uri.p);
    }
  ...
  if ((h =
	mg_get_http_header ((struct http_message *) hm,
			   "Content-Length")) != NULL)
    {
...
static void
mg_cgi_ev_handler (struct mg_connection *cgi_nc, int ev, void *ev_data)
{
      ...
      if (nc->flags & MG_F_USER_1)
	{
	  ...
	  if (len < 0 || io->len > MG_MAX_HTTP_REQUEST_SIZE)
	  ...
	  else
	    {
	      struct http_message hm;
	      struct mg_str *h;
	      mg_http_parse_headers (io->buf, io->buf + io->len, io->len,
				     &hm);
...
MG_INTERNAL void
mg_handle_cgi (struct mg_connection *nc, const char *prog,
		const struct mg_str *path_info,
		const struct http_message *hm,
		const struct mg_serve_http_opts *opts)
{
...
MG_INTERNAL void
mg_http_free_proto_data_cgi (struct mg_http_proto_data_cgi *d)
{
  if (d != NULL)
    {
      if (d->cgi_nc != NULL)
...
static void
mg_send_file_data (struct mg_connection *nc, FILE * fp)
{
  char buf[BUFSIZ];
  size_t n;
...
static void
mg_do_ssi_include (struct mg_connection *nc, struct http_message *hm,
		   ...
		   const struct mg_serve_http_opts *opts)
{
  ...
  if ((fp = mg_fopen (path, "rb")) == NULL)
  ...
  else
    {
      ...
      if (mg_match_prefix
	  (opts->ssi_pattern, strlen (opts->ssi_pattern), path) > 0)
	{
...
static void
do_ssi_exec (struct mg_connection *nc, char *tag)
{
  char cmd[BUFSIZ];
  FILE *fp;
  ...
  if (sscanf (tag, " \"%[^\"]\"", cmd) != 1)
	 ...
	 */
static void
mg_send_ssi_file (struct mg_connection *nc, struct http_message *hm,
	  ...
	  && buf[len - 2] == '-')
	{
	  ...
	  if (strncmp (p, d_include.p, d_include.len) == 0)
	  ...
	  else if (strncmp (p, d_call.p, d_call.len) == 0)
	    {
	      ...
	      mg_call (nc, NULL, MG_EV_SSI_CALL, (void *) cctx.arg.p);	/* NUL added above */
...
MG_INTERNAL void
mg_handle_ssi_request (struct mg_connection *nc,
			struct http_message *hm,
			const char *path,
			const struct mg_serve_http_opts *opts)
{
...
MG_INTERNAL int
mg_is_dav_request (const struct mg_str *s)
{
...
static int
mg_mkdir (const char *path, uint32_t mode)
{
#ifndef _WIN32
...
static void
mg_print_props (struct mg_connection *nc, const char *name, cs_stat_t * stp)
{
...
MG_INTERNAL void
mg_handle_propfind (struct mg_connection *nc, const char *path,
		    ...
		    struct mg_serve_http_opts *opts)
{
  ...
  if (S_ISDIR (stp->st_mode) &&
	 ...
	 */
MG_INTERNAL void
mg_handle_lock (struct mg_connection *nc, const char *path)
{
...
MG_INTERNAL void
mg_handle_mkcol (struct mg_connection *nc, const char *path,
		 struct http_message *hm)
{
  int status_code = 500;
...
static int
mg_remove_directory (const struct mg_serve_http_opts *opts, const char *dir)
{
  char path[MAX_PATH_SIZE];
  struct dirent *dp;
  cs_stat_t st;
...
MG_INTERNAL void
mg_handle_move (struct mg_connection *c,
		const struct mg_serve_http_opts *opts,
		const char *path, struct http_message *hm)
{
...
MG_INTERNAL void
mg_handle_delete (struct mg_connection *nc,
...
static int
mg_create_itermediate_directories (const char *path)
{
  const char *s;
...
MG_INTERNAL void
mg_handle_put (struct mg_connection *nc, const char *path,
		struct http_message *hm)
{
  struct mg_http_proto_data *pd = mg_http_get_proto_data (nc);
  cs_stat_t st;
...
static int
mg_is_ws_fragment (unsigned char flags)
{
  return (flags & 0x80) == 0 || (flags & 0x0f) == 0;
}

static int
mg_is_ws_first_fragment (unsigned char flags)
{
  return (flags & 0x80) == 0 && (flags & 0x0f) != 0;
}

static void
mg_handle_incoming_websocket_frame (struct mg_connection *nc,
...
static int
mg_deliver_websocket_data (struct mg_connection *nc)
{
...
static void
mg_send_ws_header (struct mg_connection *nc, int op, size_t len,
...
static void
mg_ws_mask_frame (struct mbuf *mbuf, struct ws_mask_ctx *ctx)
{
  size_t i;
  if (ctx->pos == 0)
    return;
...
void
mg_send_websocket_frame (struct mg_connection *nc, int op, const void *data,
			 size_t len)
{
  struct ws_mask_ctx ctx;
...
void
mg_send_websocket_framev (struct mg_connection *nc, int op,
...
void
mg_printf_websocket_frame (struct mg_connection *nc, int op,
			   const char *fmt, ...)
{
  char mem[MG_VPRINTF_BUFFER_SIZE], *buf = mem;
  va_list ap;
...
MG_INTERNAL void
mg_ws_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  mg_call (nc, nc->handler, ev, ev_data);
...
#ifndef MG_EXT_SHA1
static void
mg_hash_sha1_v (size_t num_msgs, const uint8_t * msgs[],
...
MG_INTERNAL void
mg_ws_handshake (struct mg_connection *nc, const struct mg_str *key)
{
  static const char *magic = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
  const uint8_t *msgs[2] =
    { (const uint8_t *) key->p, (const uint8_t *) magic };
...
void
mg_send_websocket_handshake2 (struct mg_connection *nc, const char *path,
...
void
mg_send_websocket_handshake3 (struct mg_connection *nc, const char *path,
			      const char *host, const char *protocol,
...
void
mg_send_websocket_handshake (struct mg_connection *nc, const char *path,
			     const char *extra_headers)
{
...
struct mg_connection *
mg_connect_ws_opt (struct mg_mgr *mgr,
		   mg_event_handler_t ev_handler,
		   struct mg_connect_opts opts,
		   ...
		   const char *extra_headers)
{
  ...
  const char *path = NULL;
  struct mg_connection *nc =
    mg_connect_http_base (mgr, ev_handler, opts, "ws://", "wss://", url,
			  &path,
			  &user, &pass, &addr);

  if (nc != NULL)
    {
      mg_send_websocket_handshake3 (nc, path, addr, protocol, extra_headers,
				    user, pass);
    }
...
struct mg_connection *
mg_connect_ws (struct mg_mgr *mgr,
		mg_event_handler_t ev_handler,
		const char *url, const char *protocol,
		const char *extra_headers)
{
...
const char *
mg_skip (const char *s, const char *end, const char *delims, struct mg_str *v)
{
  v->p = s;
...
#if MG_ENABLE_FILESYSTEM
int
mg_stat (const char *path, cs_stat_t * st)
{
#ifdef _WIN32
...
int
mg_open (const char *path, int flag, int mode)
{				/* LCOV_EXCL_LINE */
...
void
mg_base64_encode (const unsigned char *src, int src_len, char *dst)
{
  cs_base64_encode (src, src_len, dst);
}

int
mg_base64_decode (const unsigned char *s, int len, char *dst)
{
...
void *
mg_start_thread (void *(*f) (void *), void *p)
{
#ifdef WINCE
  return (void *) CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE) f, p, 0,
				NULL);
#elif defined(_WIN32)
...
void
mg_set_close_on_exec (sock_t sock)
{
#if defined(_WIN32) && !defined(WINCE)
...
void
mg_sock_addr_to_str (const union socket_address *sa, char *buf, size_t len,
		     int flags)
{
  int is_v6;
  if (buf == NULL || len <= 0)
...
#else
      if (inet_ntop (AF_INET, (void *) &sa->sin.sin_addr, buf, len - 1) ==
	  NULL)
	{
	  goto cleanup;
	}
...
void
mg_conn_addr_to_str (struct mg_connection *nc, char *buf, size_t len,
		     int flags)
{
  union socket_address sa;
  memset (&sa, 0, sizeof(sa));
...
#if MG_ENABLE_HEXDUMP
static int
mg_hexdump_n (const void *buf, int len, char *dst, int dst_len, int offset)
{
...
int
mg_hexdump (const void *buf, int len, char *dst, int dst_len)
{
  return mg_hexdump_n (buf, len, dst, dst_len, 0);
}

void
mg_hexdumpf (FILE * fp, const void *buf, int len)
{
  char tmp[80];
  int offset = 0, n;
  while (len > 0)
    {
      n = (len < 16 ? len : 16);
      mg_hexdump_n (((const char *) buf) + offset, n, tmp, sizeof(tmp),
		    offset);
      fputs (tmp, fp);
      offset += n;
      len -= n;
...
void
mg_hexdump_connection (struct mg_connection *nc, const char *path,
			const void *buf, int num_bytes, int ev)
{
  ...
  fprintf (fp, "%lu %p %s %s %s %d\n", (unsigned long) mg_time (),
	   (void *) nc, src,
	   ev == MG_EV_RECV ? "<-" : ev == MG_EV_SEND ? "->" : ev ==
	   MG_EV_ACCEPT ? "<A" : ev == MG_EV_CONNECT ? "C>" : "XX", dst,
	   num_bytes);
...
const char *
mg_next_comma_list_entry (const char *list, struct mg_str *val,
...
int
mg_match_prefix_n (const struct mg_str pattern, const struct mg_str str)
{
  const char *or_str;
  size_t len, i = 0, j = 0;
  int res;
	  ...
	  do
	    {
	      ...
	      const struct mg_str sstr =
		{ str.p + j + len, str.len - j - len };
...
DO_NOT_WARN_UNUSED MG_INTERNAL int
mg_get_errno (void)
{
#ifndef WINCE
  return errno;
...
void
mg_mbuf_append_base64_putc (char ch, void *user_data)
{
...
void
mg_mbuf_append_base64 (struct mbuf *mbuf, const void *data, size_t len)
{
...
void
mg_basic_auth_header (const char *user, const char *pass, struct mbuf *buf)
{
...
static const char *
scanto (const char *p, struct mg_str *s)
{
  s->len = getu16 (p);
  s->p = p + 2;
  return s->p + s->len;
...
MG_INTERNAL int
parse_mqtt (struct mbuf *io, struct mg_mqtt_message *mm)
{
  uint8_t header;
  size_t len = 0;
  int cmd;
  ...
  switch (cmd)
	...
	LOG (LL_DEBUG,
	     ("%d %2x %d proto [%.*s] client_id [%.*s] will_topic [%.*s] "
	      ...
	      (int) mm->client_id.len, mm->client_id.p,
	      (int) mm->will_topic.len, mm->will_topic.p,
	      (int) mm->will_message.len, mm->will_message.p,
	      (int) mm->user_name.len, mm->user_name.p,
	      (int) mm->password.len, mm->password.p));
	break;
      }
...
static void
mqtt_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  int len;
  struct mbuf *io = &nc->recv_mbuf;
...
static void
mg_mqtt_proto_data_destructor (void *proto_data)
{
  MG_FREE (proto_data);
}

void
mg_set_protocol_mqtt (struct mg_connection *nc)
{
...
void
mg_send_mqtt_handshake (struct mg_connection *nc, const char *client_id)
{
...
void
mg_send_mqtt_handshake_opt (struct mg_connection *nc, const char *client_id,
...
static void
mg_mqtt_prepend_header (struct mg_connection *nc, uint8_t cmd,
...
void
mg_mqtt_publish (struct mg_connection *nc, const char *topic,
		 uint16_t message_id, int flags, const void *data, size_t len)
{
...
void
mg_mqtt_subscribe (struct mg_connection *nc,
...
int
mg_mqtt_next_subscribe_topic (struct mg_mqtt_message *msg,
...
void
mg_mqtt_unsubscribe (struct mg_connection *nc, char **topics,
		     size_t topics_len, uint16_t message_id)
{
  size_t old_len = nc->send_mbuf.len;
...
void
mg_mqtt_connack (struct mg_connection *nc, uint8_t return_code)
{
  uint8_t unused = 0;
  mg_send (nc, &unused, 1);
  ...
  mg_mqtt_prepend_header (nc, MG_MQTT_CMD_CONNACK, 0, 2);
	 ...
	 */
static void
mg_send_mqtt_short_command (struct mg_connection *nc, uint8_t cmd,
			    uint16_t message_id)
{
  uint16_t message_id_net = htons (message_id);
  mg_send (nc, &message_id_net, 2);
...
void
mg_mqtt_puback (struct mg_connection *nc, uint16_t message_id)
{
  mg_send_mqtt_short_command (nc, MG_MQTT_CMD_PUBACK, message_id);
}

void
mg_mqtt_pubrec (struct mg_connection *nc, uint16_t message_id)
{
  mg_send_mqtt_short_command (nc, MG_MQTT_CMD_PUBREC, message_id);
}

void
mg_mqtt_pubrel (struct mg_connection *nc, uint16_t message_id)
{
  mg_send_mqtt_short_command (nc, MG_MQTT_CMD_PUBREL, message_id);
}

void
mg_mqtt_pubcomp (struct mg_connection *nc, uint16_t message_id)
{
  mg_send_mqtt_short_command (nc, MG_MQTT_CMD_PUBCOMP, message_id);
}

void
mg_mqtt_suback (struct mg_connection *nc, uint8_t * qoss, size_t qoss_len,
		uint16_t message_id)
{
  size_t i;
  uint16_t message_id_net = htons (message_id);
...
void
mg_mqtt_unsuback (struct mg_connection *nc, uint16_t message_id)
{
...
void
mg_mqtt_disconnect (struct mg_connection *nc)
{
...
static void
mg_mqtt_session_init (struct mg_mqtt_broker *brk,
		      struct mg_mqtt_session *s, struct mg_connection *nc)
{
  s->brk = brk;
...
static void
mg_mqtt_add_session (struct mg_mqtt_session *s)
{
  LIST_INSERT_HEAD (&s->brk->sessions, s, link);
}

static void
mg_mqtt_remove_session (struct mg_mqtt_session *s)
{
  LIST_REMOVE (s, link);
}

static void
mg_mqtt_destroy_session (struct mg_mqtt_session *s)
{
  size_t i;
...
static void
mg_mqtt_close_session (struct mg_mqtt_session *s)
{
...
void
mg_mqtt_broker_init (struct mg_mqtt_broker *brk, void *user_data)
{
...
static void
mg_mqtt_broker_handle_connect (struct mg_mqtt_broker *brk,
				struct mg_connection *nc)
{
  struct mg_mqtt_session *s =
    (struct mg_mqtt_session *) calloc (1, sizeof*s);
  if (s == NULL)
    {
      /* LCOV_EXCL_START */
...
static void
mg_mqtt_broker_handle_subscribe (struct mg_connection *nc,
				 struct mg_mqtt_message *msg)
{
  ...
  ss->subscriptions =
    (struct mg_mqtt_topic_expression *) realloc (ss->subscriptions,
						 sizeof(*ss->subscriptions) *
						 qoss_len);
  for (pos = 0;
	(pos = mg_mqtt_next_subscribe_topic (msg, &topic, &qos, pos)) != -1;
  ...
  mg_mqtt_suback (nc, qoss, qoss_len, msg->message_id);
	 ...
	 */
static int
mg_mqtt_match_topic_expression (const char *exp, const struct mg_str *topic)
{
...
static void
mg_mqtt_broker_handle_publish (struct mg_mqtt_broker *brk,
				struct mg_mqtt_message *msg)
{
  struct mg_mqtt_session *s;
  size_t i;
      ...
      for (i = 0; i < s->num_subscriptions; i++)
	{
	  if (mg_mqtt_match_topic_expression (s->subscriptions[i].topic,
	      ...
	      if (p == NULL)
		{
	      ...
	      mg_mqtt_publish (s->nc, p, 0, 0, msg->payload.p,
				msg->payload.len);
	      if (p != buf)
		{
		  MG_FREE (p);
...
void
mg_mqtt_broker (struct mg_connection *nc, int ev, void *data)
{
...
mg_mqtt_next (struct mg_mqtt_broker *brk, struct mg_mqtt_session *s)
{
...
int
mg_dns_parse_record_data (struct mg_dns_message *msg,
...
int
mg_dns_insert_header (struct mbuf *io, size_t pos, struct mg_dns_message *msg)
{
...
int
mg_dns_copy_questions (struct mbuf *io, struct mg_dns_message *msg)
{
...
int
mg_dns_encode_name (struct mbuf *io, const char *name, size_t len)
{
  const char *s;
  unsigned char n;
  size_t pos = io->len;
...
int
mg_dns_encode_record (struct mbuf *io, struct mg_dns_resource_record *rr,
		      ...
		      size_t rlen)
{
  ...
  if (rr->kind == MG_DNS_ANSWER)
      ...
      if (rr->rtype == MG_DNS_CNAME_RECORD)
	{
	  ...
	  mbuf_append (io, &u16, 2);
	  if ((clen =
		mg_dns_encode_name (io, (const char *) rdata, rlen)) == -1)
	    {
	      return -1;
	    }
	  u16 = clen;
...
void
mg_send_dns_query (struct mg_connection *nc, const char *name, int query_type)
{
...
int
mg_parse_dns (const char *buf, int len, struct mg_dns_message *msg)
{
...
size_t
mg_dns_uncompress_name (struct mg_dns_message * msg, struct mg_str * name,
...
static void
dns_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  struct mbuf *io = &nc->recv_mbuf;
  struct mg_dns_message msg;
...
void
mg_set_protocol_dns (struct mg_connection *nc)
{
...
struct mg_dns_reply
mg_dns_create_reply (struct mbuf *io, struct mg_dns_message *msg)
{
...
void
mg_dns_send_reply (struct mg_connection *nc, struct mg_dns_reply *r)
{
...
int
mg_dns_reply_record (struct mg_dns_reply *reply,
  ...
  if (name == NULL)
    {
      name = rname;
      rname[511] = 0;
      mg_dns_uncompress_name (msg, &question->name, rname,
			      sizeof(rname) - 1);
    }
...
struct mg_resolve_async_request
{
  ...
  int retries;
	 ...
	 */
static int
mg_get_ip_address_of_nameserver (char *name, size_t name_len)
{
  int ret = -1;
  ...
  if ((err = RegOpenKeyExW (HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &hKey)) !=
      ...
      for (ret = -1, i = 0; 1; i++)
	{
	  ...
	  if (RegOpenKeyExW (hKey, subkey, 0, KEY_READ, &hSub) ==
	      ...
	      &&
	      (RegQueryValueExW
		(hSub, L"NameServer", 0, &type, (void *) value,
		&len) == ERROR_SUCCESS
...
int
mg_resolve_from_hosts_file (const char *name, union socket_address *usa)
{
#if MG_ENABLE_FILESYSTEM
...
static void
mg_resolve_async_eh (struct mg_connection *nc, int ev, void *data)
{
...
int
mg_resolve_async (struct mg_mgr *mgr, const char *name, int query,
...
int
mg_resolve_async_opt (struct mg_mgr *mgr, const char *name, int query,
		      mg_resolve_callback_t cb, void *data,
		      struct mg_resolve_async_opts opts)
{
  struct mg_resolve_async_request *req;
  ...
  if (nameserver == NULL && mg_dns_server[0] == '\0' &&
      mg_get_ip_address_of_nameserver (mg_dns_server,
					sizeof(mg_dns_server)) == -1)
    {
...
void
mg_coap_free_options (struct mg_coap_message *cm)
{
...
mg_coap_add_option (struct mg_coap_message *cm,
		    uint32_t number, char *value, size_t len)
{
  ...
  return new_option;
	 ...
	 */
static char *
coap_parse_header (char *ptr, struct mbuf *io, struct mg_coap_message *cm)
{
  ...
  return ptr;
	 ...
	 */
static char *
coap_get_token (char *ptr, struct mbuf *io, struct mg_coap_message *cm)
{
  if (cm->token.len != 0)
    {
  ...
  return ptr;
	 ...
	 */
static int
coap_get_ext_opt (char *ptr, struct mbuf *io, uint16_t * opt_info)
{
  int ret = 0;
  ...
  return ret;
	 ...
	 */
static char *
coap_get_options (char *ptr, struct mbuf *io, struct mg_coap_message *cm)
{
  uint16_t prev_opt = 0;
...
uint32_t
mg_coap_parse (struct mbuf * io, struct mg_coap_message * cm)
{
  char *ptr;
  ...
  return cm->flags;
	 ...
	 */
static size_t
coap_get_ext_opt_size (uint32_t value)
{
  int ret = 0;
  ...
  return ret;
	 ...
	 */
static int
coap_split_opt (uint32_t value, uint8_t * base, uint16_t * ext)
{
  int ret = 0;
  ...
  return ret;
	 ...
	 */
static char *
coap_add_uint16 (char *ptr, uint16_t val)
{
  *ptr = val >> 8;
  ptr++;
  *ptr = val & 0x00FF;
  ...
  return ptr;
	 ...
	 */
static char *
coap_add_opt_info (char *ptr, uint16_t val, size_t len)
{
  ...
  return ptr;
	 ...
	 */
static uint32_t
coap_calculate_packet_size (struct mg_coap_message *cm, size_t * len)
{
...
uint32_t
mg_coap_compose (struct mg_coap_message * cm, struct mbuf * io)
{
  struct mg_coap_option *opt;
  ...
  *ptr = (cm->code_class << 5) | (cm->code_detail);
  ...
  while (opt != NULL)
	...
	coap_split_opt (opt->number - prev_opt_number, &delta_base,
			&delta_ext);
...
uint32_t
mg_coap_send_message (struct mg_connection * nc, struct mg_coap_message * cm)
{
...
uint32_t
mg_coap_send_ack (struct mg_connection * nc, uint16_t msg_id)
{
...
static void
coap_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  struct mbuf *io = &nc->recv_mbuf;
  struct mg_coap_message cm;
  uint32_t parse_res;
  ...
  switch (ev)
	 ...
	 */
int
mg_set_protocol_coap (struct mg_connection *nc)
{
  /* supports UDP only */
...
static void
mg_tun_init_client (struct mg_tun_client *client, struct mg_mgr *mgr,
...
void
mg_tun_log_frame (struct mg_tun_frame *frame)
{
  LOG (LL_DEBUG, ("Got TUN frame: type=0x%x, flags=0x%x stream_id=0x%lx, "
		  "len=%zu",
		  frame->type, frame->flags, frame->stream_id,
		  frame->body.len));
#if MG_ENABLE_HEXDUMP
  {
    char hex[512];
...
static void
mg_tun_close_all (struct mg_tun_client *client)
{
  struct mg_connection *nc;
  for (nc = client->mgr->active_connections; nc != NULL; nc = nc->next)
    {
...
static void
mg_tun_client_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  ...
  switch (ev)
	...
	if (err)
	  {
	    LOG (LL_ERROR,
		 ("Cannot connect to the tunnel dispatcher: %d", err));
	  }
	else
	  {
	...
	if (hm->resp_code != 200)
	  {
	    LOG (LL_ERROR,
		 ("Tunnel dispatcher reply non-OK status code %d",
		  hm->resp_code));
	  }
	break;
      }
	...
	if (tc == NULL)
	  {
	    if (frame.body.len > 0)
	      {
		LOG (LL_DEBUG,
		     ("Got frame after receiving end has been closed"));
	      }
	    break;
	  }
	if (frame.body.len > 0)
	...
	if (frame.flags & MG_TUN_F_END_STREAM)
	  {
	    LOG (LL_DEBUG,
		 ("Closing tunneled connection because got end of stream "
		  "from other end"));
...
static void
mg_tun_do_reconnect (struct mg_tun_client *client)
{
...
void
mg_tun_reconnect_ev_handler (struct mg_connection *nc, int ev, void *ev_data)
{
  struct mg_tun_client *client = (struct mg_tun_client *) nc->user_data;
  (void) ev_data;
...
static void
mg_tun_reconnect (struct mg_tun_client *client, int timeout)
{
  if (client->reconnect == NULL)
    {
      client->reconnect =
	mg_add_sock (client->mgr, INVALID_SOCKET,
...
static struct mg_tun_client *
mg_tun_create_client (struct mg_mgr *mgr,
		      const char *dispatcher, struct mg_tun_ssl_opts ssl)
{
...
void
mg_tun_destroy_client (struct mg_tun_client *client)
{
  /*
   *  NOTE:
...
mg_tun_do_bind (struct mg_tun_client *client,
		mg_event_handler_t handler, struct mg_bind_opts opts)
{
...
struct mg_connection *
mg_tun_bind_opt (struct mg_mgr *mgr,
		 const char *dispatcher,
		 mg_event_handler_t handler, struct mg_bind_opts opts)
{
#if MG_ENABLE_SSL
  struct mg_tun_ssl_opts ssl =
    { opts.ssl_cert, opts.ssl_key, opts.ssl_ca_cert };
#else
...
int
mg_tun_parse_frame (void *data, size_t len, struct mg_tun_frame *frame)
{
...
void
mg_tun_send_frame (struct mg_connection *ws, uint32_t stream_id,
		   uint8_t type, uint8_t flags, struct mg_str msg)
{
...
static uint64_t
mg_get_sec (uint64_t val)
{
  return (val & 0xFFFFFFFF00000000) >> 32;
}

static uint64_t
mg_get_usec (uint64_t val)
{
...
static void
mg_ntp_to_tv (uint64_t val, struct timeval *tv)
{
  uint64_t tmp;
  tmp = mg_get_sec (val);
...
static void
mg_get_ntp_ts (const char *ntp, uint64_t * val)
{
  uint32_t tmp;
...
void
mg_sntp_send_request (struct mg_connection *c)
{
  char buf[48] = { 0 };
  /*
   * header - 8 bit:
...
#ifndef MG_SNMP_NO_DELAY_CORRECTION
static uint64_t
mg_calculate_delay (uint64_t t1, uint64_t t2, uint64_t t3)
{
...
MG_INTERNAL int
mg_sntp_parse_reply (const char *buf, int len, struct mg_sntp_message *msg)
{
  uint8_t hdr;
  ...
  msg->time =
    (double) tv.tv_sec + (((double) tv.tv_usec + delay) / 1000000.0);

  return 0;
}

static void
mg_sntp_handler (struct mg_connection *c, int ev, void *ev_data)
{
  struct mbuf *io = &c->recv_mbuf;
  struct mg_sntp_message msg;
...
int
mg_set_protocol_sntp (struct mg_connection *c)
{
...
struct mg_connection *
mg_sntp_connect (struct mg_mgr *mgr,
		 mg_event_handler_t event_handler,
...
static void
mg_sntp_util_ev_handler (struct mg_connection *c, int ev, void *ev_data)
{
...
struct mg_connection *
mg_sntp_get_time (struct mg_mgr *mgr,
		  mg_event_handler_t event_handler,
		  const char *sntp_server_name)
{
...
#ifndef __TI_COMPILER_VERSION__
int
_gettimeofday_r (struct _reent *r, struct timeval *tp, void *tzp)
{
#else
int
...
void
fprint_str (FILE * fp, const char *str)
{
  while (*str != '\0')
    {
      if (*str == '\n')
...
void
_not_implemented (const char *what)
{
...
int
fs_slfs_open (const char *pathname, int flags, mode_t mode)
{
  int fd;
...
ssize_t
fs_slfs_read (int fd, void *buf, size_t count)
{
  struct sl_fd_info *fi = &s_sl_fds[fd];
  if (fi->fh <= 0)
    return set_errno (EBADF);
...
ssize_t
fs_slfs_write (int fd, const void *buf, size_t count)
{
  struct sl_fd_info *fi = &s_sl_fds[fd];
  if (fi->fh <= 0)
    return set_errno (EBADF);
...
int
fs_slfs_stat (const char *pathname, struct stat *s)
{
  SlFsFileInfo_t sl_fi;
  /*
   * Apply path manipulations again, in case we got here directly
...
int
fs_slfs_fstat (int fd, struct stat *s)
{
  struct sl_fd_info *fi = &s_sl_fds[fd];
  if (fi->fh <= 0)
    return set_errno (EBADF);
...
off_t
fs_slfs_lseek (int fd, off_t offset, int whence)
{
  if (s_sl_fds[fd].fh <= 0)
    return set_errno (EBADF);
  switch (whence)
...
int
fs_slfs_unlink (const char *pathname)
{
  /*
   * Apply path manipulations again, in case we got here directly
   * (via TI libc's "add_device").
...
int
fs_slfs_rename (const char *from, const char *to)
{
  return set_errno (ENOTSUP);
}

void
fs_slfs_set_new_file_size (const char *name, size_t size)
{
  int i;
  for (i = 0; i < MAX_OPEN_SLFS_FILES; i++)
    {
...
static const char *
drop_dir (const char *fname, bool * is_slfs)
{
  if (is_slfs != NULL)
    {
...
#if MG_TI_NO_HOST_INTERFACE
int
open (const char *pathname, unsigned flags, int mode)
{
#else
int
...
int
_stat (const char *pathname, struct stat *st)
{
  int res = -1;
  bool is_sl;
  const char *fname = drop_dir (pathname, &is_sl);
...
#if MG_TI_NO_HOST_INTERFACE
int
read (int fd, char *buf, unsigned count)
{
#else
ssize_t
...
#if MG_TI_NO_HOST_INTERFACE
int
write (int fd, const char *buf, unsigned count)
{
#else
ssize_t
...
int
rename (const char *frompath, const char *topath)
{
  int r = -1;
...
bool
mg_start_task (int priority, int stack_size, mg_init_cb mg_init)
{
...
void
mg_run_in_task (void (*cb) (struct mg_mgr * mgr, void *arg), void *cb_arg)
{
...
void
mg_sl_if_connect_tcp (struct mg_connection *nc,
		      const union socket_address *sa)
{
  int proto = 0;
  if (nc->flags & MG_F_SSL)
...
void
mg_sl_if_connect_udp (struct mg_connection *nc)
{
...
int
mg_sl_if_listen_tcp (struct mg_connection *nc, union socket_address *sa)
{
  int proto = 0;
  if (nc->flags & MG_F_SSL)
    proto = SL_SEC_SOCKET;
...
int
mg_sl_if_listen_udp (struct mg_connection *nc, union socket_address *sa)
{
  sock_t sock = mg_open_listening_socket (sa, SOCK_DGRAM, 0);
  if (sock == INVALID_SOCKET)
...
void
mg_sl_if_tcp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
}

void
mg_sl_if_udp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
}

void
mg_sl_if_recved (struct mg_connection *nc, size_t len)
{
  (void) nc;
  (void) len;
}
...
int
mg_sl_if_create_conn (struct mg_connection *nc)
{
  (void) nc;
  return 1;
}
...
void
mg_sl_if_destroy_conn (struct mg_connection *nc)
{
  if (nc->sock == INVALID_SOCKET)
    return;
...
mg_mgr_handle_conn (struct mg_connection *nc, int fd_flags, double now)
{
  ...
  if (nc->flags & MG_F_CONNECTING)
      ...
      else
	{
	  ...
	  if (fd_flags & _MG_F_FD_CAN_WRITE)
	    {
	      nc->err =
		sl_Connect (nc->sock, &nc->sa.sa, sizeof(nc->sa.sin));
  ...
  DBG (("%p after fd=%d nc_flags=%lu rmbl=%d smbl=%d", nc, nc->sock,
	nc->flags, (int) nc->recv_mbuf.len, (int) nc->send_mbuf.len));
								...
								/* Associate a socket to a connection. */
void
mg_sl_if_sock_set (struct mg_connection *nc, sock_t sock)
{
  mg_set_non_blocking_mode (sock);
  nc->sock = sock;
  DBG (("%p %d", nc, sock));
...
void
mg_sl_if_init (struct mg_iface *iface)
{
  (void) iface;
...
void
mg_sl_if_free (struct mg_iface *iface)
{
  (void) iface;
}

void
mg_sl_if_add_conn (struct mg_connection *nc)
{
  (void) nc;
}

void
mg_sl_if_remove_conn (struct mg_connection *nc)
{
  (void) nc;
}

time_t
mg_sl_if_poll (struct mg_iface *iface, int timeout_ms)
{
  struct mg_mgr *mgr = iface->mgr;
  double now = mg_time ();
  double min_timer;
  ...
  if (num_fds > 0)
    {
      num_ev =
	sl_Select ((int) max_fd + 1, &read_set, &write_set, &err_set, &tv);
    }
...
void
mg_sl_if_get_conn_addr (struct mg_connection *nc, int remote,
			union socket_address *sa)
{
...
void
sl_restart_cb (struct mg_mgr *mgr)
{
  /*
...
bool
pem_to_der (const char *pem_file, const char *der_file)
{
  bool ret = false;
...
static char *
sl_pem2der (const char *pem_file)
{
  ...
  int l =
    mg_asprintf (&der_file, 0, "SL:%.*s.der", (int) (pem_ext - pem_file),
		 pem_file);
  if (der_file == NULL)
    return NULL;
  bool result = false;
...
int
sl_set_ssl_opts (struct mg_connection *nc)
{
  int err;
  struct mg_ssl_if_ctx *ctx = (struct mg_ssl_if_ctx *) nc->ssl_if_data;
	  ...
	  if (ssl_cert != NULL && ssl_key != NULL)
	    {
	      err = sl_SetSockOpt (nc->sock, SL_SOL_SOCKET,
				   ...
				   ssl_cert, strlen (ssl_cert));
	      LOG (LL_INFO,
		   ("CERTIFICATE_FILE_NAME %s -> %d", ssl_cert, err));
	      err =
		sl_SetSockOpt (nc->sock, SL_SOL_SOCKET,
...
void
mg_lwip_set_keepalive_params (struct mg_connection *nc, int idle,
...
static err_t
mg_lwip_tcp_conn_cb (void *arg, struct tcp_pcb *tpcb, err_t err)
{
...
static void
mg_lwip_tcp_error_cb (void *arg, err_t err)
{
  struct mg_connection *nc = (struct mg_connection *) arg;
...
static err_t
mg_lwip_tcp_recv_cb (void *arg, struct tcp_pcb *tpcb,
...
static void
mg_lwip_handle_recv_tcp (struct mg_connection *nc)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
static err_t
mg_lwip_tcp_sent_cb (void *arg, struct tcp_pcb *tpcb, u16_t num_sent)
{
  struct mg_connection *nc = (struct mg_connection *) arg;
...
void
mg_lwip_if_connect_tcp (struct mg_connection *nc,
			const union socket_address *sa)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
static void
mg_lwip_udp_recv_cb (void *arg, struct udp_pcb *pcb, struct pbuf *p,
...
{
  struct mg_connection *nc = (struct mg_connection *) arg;
  DBG (("%p %s:%u %p %u %u", nc, IPADDR_NTOA (addr), port, p, p->ref,
	p->len));
...
static void
mg_lwip_recv_common (struct mg_connection *nc, struct pbuf *p)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  if (cs->rx_chain == NULL)
    {
...
static void
mg_lwip_handle_recv_udp (struct mg_connection *nc)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  /*
...
void
mg_lwip_if_connect_udp (struct mg_connection *nc)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
void
mg_lwip_accept_conn (struct mg_connection *nc, struct tcp_pcb *tpcb)
{
  union socket_address sa;
...
static err_t
mg_lwip_accept_cb (void *arg, struct tcp_pcb *newtpcb, err_t err)
{
  struct mg_connection *lc = (struct mg_connection *) arg;
  (void) err;
  DBG (("%p conn %p from %s:%u", lc, newtpcb,
	IPADDR_NTOA (&newtpcb->remote_ip), newtpcb->remote_port));
...
int
mg_lwip_if_listen_tcp (struct mg_connection *nc, union socket_address *sa)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  struct tcp_pcb *tpcb = TCP_NEW ();
  ip_addr_t *ip = (ip_addr_t *) & sa->sin.sin_addr.s_addr;
...
int
mg_lwip_if_listen_udp (struct mg_connection *nc, union socket_address *sa)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  struct udp_pcb *upcb = udp_new ();
  ip_addr_t *ip = (ip_addr_t *) & sa->sin.sin_addr.s_addr;
...
int
mg_lwip_tcp_write (struct mg_connection *nc, const void *data, uint16_t len)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  struct tcp_pcb *tpcb = cs->pcb.tcp;
  ...
  if (len == 0)
    {
      DBG (("%p no buf avail %u %u %u %p %p", tpcb, tpcb->acked,
	    tpcb->snd_buf, tpcb->snd_queuelen, tpcb->unsent, tpcb->unacked));
      tcp_output (tpcb);
      return 0;
    }
  /*
...
static void
mg_lwip_send_more (struct mg_connection *nc)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
void
mg_lwip_if_tcp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
  mg_lwip_mgr_schedule_poll (nc->mgr);
}
...
void
mg_lwip_if_udp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
void
mg_lwip_if_recved (struct mg_connection *nc, size_t len)
{
  if (nc->flags & MG_F_UDP)
    return;
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
int
mg_lwip_if_create_conn (struct mg_connection *nc)
{
...
void
mg_lwip_if_destroy_conn (struct mg_connection *nc)
{
  if (nc->sock == INVALID_SOCKET)
    return;
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
...
void
mg_lwip_if_get_conn_addr (struct mg_connection *nc, int remote,
			  union socket_address *sa)
{
  memset (sa, 0, sizeof(*sa));
  if (nc->sock == INVALID_SOCKET)
...
void
mg_lwip_if_sock_set (struct mg_connection *nc, sock_t sock)
{
  nc->sock = sock;
}
...
void
mg_lwip_post_signal (enum mg_sig_type sig, struct mg_connection *nc)
{
...
void
mg_ev_mgr_lwip_process_signals (struct mg_mgr *mgr)
{
  struct mg_ev_mgr_lwip_data *md =
...
void
mg_lwip_if_init (struct mg_iface *iface)
{
...
void
mg_lwip_if_free (struct mg_iface *iface)
{
  MG_FREE (iface->data);
  iface->data = NULL;
}
...
void
mg_lwip_if_add_conn (struct mg_connection *nc)
{
  (void) nc;
}

void
mg_lwip_if_remove_conn (struct mg_connection *nc)
{
  struct mg_ev_mgr_lwip_data *md =
    (struct mg_ev_mgr_lwip_data *) nc->iface->data;
...
time_t
mg_lwip_if_poll (struct mg_iface *iface, int timeout_ms)
{
  struct mg_mgr *mgr = iface->mgr;
  int n = 0;
  double now = mg_time ();
...
uint32_t
mg_lwip_get_poll_delay_ms (struct mg_mgr * mgr)
{
  struct mg_connection *nc;
  double now = mg_time ();
  double min_timer = 0;
...
void
mg_lwip_ssl_do_hs (struct mg_connection *nc)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  int server_side = (nc->listener != NULL);
...
void
mg_lwip_ssl_send (struct mg_connection *nc)
{
  if (nc->sock == INVALID_SOCKET)
    {
      DBG (("%p invalid socket", nc));
...
void
mg_lwip_ssl_recv (struct mg_connection *nc)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) nc->sock;
  ...
  while (nc->recv_mbuf.len < MG_LWIP_SSL_RECV_MBUF_LIMIT)
	...
	return;
      ...
      DBG (("%p %p SSL_read %u = %d", nc, cs->rx_chain, MG_LWIP_SSL_IO_SIZE,
	    ret));
      if (ret <= 0)
	{
	  free (buf);
...
ssize_t
kr_send (int fd, const void *buf, size_t len)
{
...
ssize_t
kr_recv (int fd, void *buf, size_t len)
{
  struct mg_lwip_conn_state *cs = (struct mg_lwip_conn_state *) fd;
  struct pbuf *seg = cs->rx_chain;
  if (seg == NULL)
...
int
_wstati64 (const wchar_t * path, cs_stat_t * st)
{
...
int
mg_pic32_if_create_conn (struct mg_connection *nc)
{
  (void) nc;
  return 1;
}
...
void
mg_pic32_if_recved (struct mg_connection *nc, size_t len)
{
  (void) nc;
  (void) len;
}
...
void
mg_pic32_if_add_conn (struct mg_connection *nc)
{
  (void) nc;
}

void
mg_pic32_if_init (struct mg_iface *iface)
{
  (void) iface;
...
void
mg_pic32_if_free (struct mg_iface *iface)
{
  (void) iface;
}

void
mg_pic32_if_remove_conn (struct mg_connection *nc)
{
  (void) nc;
}

void
mg_pic32_if_destroy_conn (struct mg_connection *nc)
{
  if (nc->sock == INVALID_SOCKET)
    return;
  /* For UDP, only close outgoing sockets or listeners. */
...
int
mg_pic32_if_listen_udp (struct mg_connection *nc, union socket_address *sa)
{
  nc->sock =
    TCPIP_UDP_ServerOpen (sa->sin.sin_family ==
			  AF_INET ? IP_ADDRESS_TYPE_IPV4 :
			  IP_ADDRESS_TYPE_IPV6, ntohs (sa->sin.sin_port),
			  sa->sin.sin_addr.s_addr ==
			  0 ? 0 : (IP_MULTI_ADDRESS *) & sa->sin);
  if (nc->sock == INVALID_SOCKET)
    {
      return -1;
    }
...
void
mg_pic32_if_udp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
}

void
mg_pic32_if_tcp_send (struct mg_connection *nc, const void *buf, size_t len)
{
  mbuf_append (&nc->send_mbuf, buf, len);
}

int
mg_pic32_if_listen_tcp (struct mg_connection *nc, union socket_address *sa)
{
  nc->sock =
    TCPIP_TCP_ServerOpen (sa->sin.sin_family ==
			  AF_INET ? IP_ADDRESS_TYPE_IPV4 :
			  IP_ADDRESS_TYPE_IPV6, ntohs (sa->sin.sin_port),
			  sa->sin.sin_addr.s_addr ==
			  0 ? 0 : (IP_MULTI_ADDRESS *) & sa->sin);
...
static void
mg_handle_send (struct mg_connection *nc)
{
  uint16_t bytes_written = 0;
  if (nc->flags & MG_F_UDP)
    {
      if (!TCPIP_UDP_RemoteBind ((UDP_SOCKET) nc->sock,
				 nc->sa.sin.sin_family ==
				 AF_INET ? IP_ADDRESS_TYPE_IPV4 :
				 IP_ADDRESS_TYPE_IPV6,
				 ntohs (nc->sa.sin.sin_port),
				 (IP_MULTI_ADDRESS *) & nc->sa.sin))
	{
	  nc->flags |= MG_F_CLOSE_IMMEDIATELY;
	  return;
	}
...
static void
mg_handle_recv (struct mg_connection *nc)
{
...
time_t
mg_pic32_if_poll (struct mg_iface *iface, int timeout_ms)
{
  struct mg_mgr *mgr = iface->mgr;
  double now = mg_time ();
  struct mg_connection *nc, *tmp;
...
void
mg_pic32_if_sock_set (struct mg_connection *nc, sock_t sock)
{
  nc->sock = sock;
}

void
mg_pic32_if_get_conn_addr (struct mg_connection *nc, int remote,
			   union socket_address *sa)
{
...
void
mg_pic32_if_connect_tcp (struct mg_connection *nc,
			 const union socket_address *sa)
{
  nc->sock =
    TCPIP_TCP_ClientOpen (sa->sin.sin_family ==
			  AF_INET ? IP_ADDRESS_TYPE_IPV4 :
			  IP_ADDRESS_TYPE_IPV6, ntohs (sa->sin.sin_port),
			  (IP_MULTI_ADDRESS *) & sa->sin);
  nc->err = (nc->sock == INVALID_SOCKET) ? -1 : 0;
}

void
mg_pic32_if_connect_udp (struct mg_connection *nc)
{
